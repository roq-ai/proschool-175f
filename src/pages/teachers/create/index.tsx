...
function TeacherCreatePage() {
  ...
  const formik = useFormik<TeacherInterface>({
    initialValues: {
      subject: '',
      hired_date: new Date(new Date().toDateString()),
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <FormControl id="hired_date" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Hired Date
            </FormLabel>
            <DatePicker
              selected={formik.values?.hired_date ? new Date(formik.values?.hired_date) : null}
              onChange={(value: Date) => formik.setFieldValue('hired_date', value)}
            />
          </FormControl>
